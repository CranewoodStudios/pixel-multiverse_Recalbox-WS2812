cat > /recalbox/share/pixel-multiverse/pm_daemon.py <<'PY'
#!/usr/bin/env python3
...
BRIGHT_LIMIT = 100                 # cap brightness (0..255)
FPS = 60
...
def find_serial_port():
    ...
    try:
        for name in os.listdir(byid):
            low = name.lower()
            if ("2040" in low or "pimoroni" in low or "micropython" in low or "plasma" in low):
    ...
    for p in ("/dev/ttyACM1", "/dev/ttyACM2"):
        if os.path.exists(p):
            try:
...
def main():
    ...
    if not port:
        log("ERROR: no serial port found (is the Plasma 2040 connected & code.py running?)")
        return
